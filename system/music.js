...
module.exports = {
  ...
  run: async function (client, message, args) {
   ...
   var serverQueue = message.client.queue.get(message.guild.id);
    ...
    if (!song){
      if (!online.afk) {
        sendError("Leaving the voice channel because I think there are no songs in the queue. If you like the bot stay 24/7 in voice channel run `!afk`nnThank you for using my code! [GitHub](https://github.com/SudhanPlayz/Discord-MusicBot)", message.channel)
...
stream.on('error', function(er)  {
      if (er) {
        ...
        queue.songs.shift();
        play(queue.songs[0]);
  	  return sendError(`An unexpected error has occurred.nPossible type `${er}``, message.channel)
          }
        }
    });
}
